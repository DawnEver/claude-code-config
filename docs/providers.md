...
(see "Auto-rewriting `codex_config.toml`" below).

**GMI has no Codex launcher, by design.** `inject-codex-providers.mjs` writes
`wire_api = "responses"` for every provider, which suits DeepSeek but made `cogmi`
return 422 on every call. The cause is not the wire_api value: GMI serves the
**Anthropic** protocol, and Codex speaks only OpenAI wire formats (`responses` /
`chat`), so neither setting can bridge them. `cogmi` was removed rather than
re-pointed. GMI is reachable through `ccgmi` on the Claude side, where the protocol
matches. If a provider ever needs a non-`responses` wire format, that is the point to
make `wire_api` a per-provider field in `providers.<name>` — today it is hardcoded.
...
- **Additional `codex` aliases (`co`, `cokm`)** — the foundation supports them. To
  ...
  - `cokm` (codex + Kimi): add a `providers.kimi` block (already present in
    ...
    needs the alias entry). Until then, only `cods` is installed.
