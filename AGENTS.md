...
- `codex_config.toml`: Codex config (model, sandbox, TUI) — synced via OneDrive, gitignored. User maintains a `[model_providers.<id>]` block per provider (e.g. `deepseek` / `gmi`) for `cods`/`cogmi` to actually resolve their `env_key` and `wire_api = "responses"` precondition; see `.claude/rules/rem/providers.md` § Codex side
...
- `cods` / `cogmi` — Codex launchers (DeepSeek / GMI), same `providers.<name>` block. `claude_env_settings.json` is the single source of truth for both hosts — see `.claude/rules/rem/providers.md`.
