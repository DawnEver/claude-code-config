...
- `cc-market/`: Community plugin marketplace (gitignored, cloned by setup) â€?provides `takeover` for multi-model orchestration
- `claude_settings.json`: Env vars, permissions, hooks (gitignored, secrets not tracked)
- `claude_settings.template.json`: Template for new clones â€?auto-copied to `claude_settings.json` by setup
- `claude_env_settings.json`: API keys per provider (gitignored, secrets not tracked)
- `claude_env_settings.template.json`: Desensitized provider template â€?auto-copied to `claude_env_settings.json` by setup
- `keybindings.json`: Claude Code keybindings â€?synced to `~/.claude/keybindings.json`
- `GLOBAL-AGENTS.md`: Global guidelines

### Provider Switching
- `cc` â€?launch Claude Code with official Claude Pro subscription (clears all provider env vars)
- `ccds` â€?launch Claude Code via DeepSeek API (reads `claude_env_settings.json`)
