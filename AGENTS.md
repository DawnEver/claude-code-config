...
- `codex_config.toml`: Codex config (model, sandbox, TUI) — synced via OneDrive, gitignored. Setup auto-injects a `[model_providers.<id>]` block per codex-facing provider (generated by `scripts/setup/inject-codex-providers.mjs` from the shared `providers.<name>` registry) so `cods`/`cogmi` resolve their `env_key` and `wire_api = "responses"` precondition; see `docs/providers.md` § Codex side
- `models.json`: Codex model catalog (context window, reasoning levels, image input) — same injector generates it from `providers.<name>.models`, linked to `~/.codex/models.json` so Codex accepts the deepseek-v4-* (etc.) models. Synced via OneDrive, gitignored
